...
# Delete attachment from source sheet after successful transfer
def delete_attachment(sheet_id, attachment_id):
    access_token = session.get('ACCESS_TOKEN')
    if not access_token:
    url = f"https://api.smartsheet.com/2.0/sheets/{sheet_id}/attachments/{attachment_id}"
    headers = {"Authorization": f"Bearer {access_token}"}
    try:
        ...
        response.raise_for_status()
        log_message(f"SUCCESS: Deleted attachment ID {attachment_id} from sheet {sheet_id}")
    except requests.exceptions.RequestException as e:
        log_message(f"ERROR: Failed to delete attachment ID {attachment_id} from sheet {sheet_id}: {e}")
...
def transfer_attachments():
    try:
        ...
        for attachment in selected_attachments:
            ...
            if not file_name:
                log_message(f"ERROR: file_name is None for attachment ID {attachment_id}")
                continue
            ...
            if existing_attachment_id:
                ...
                return jsonify({"prompt": "Attachment already exists. Do you want to upload a new version?", "attachmentId": existing_attachment_id, "fileName": file_name})
